...
def test_real_pi_steering_and_stop_at_a_running_tool(monkeypatch, repo_root, stop):
    ...
    with ExitStack() as cleanup:
        ...
        job = {'chat_id': 'chat-test', 'turn_id': 'turn-test', 'model': 'test-model', 'prompt': 'Run the blocking tool'}
        container = client.containers.create(
            'nautionette/pi-base:dev', entrypoint='node', command=['/workspace/chat-model.mjs'],
            environment={
            ...
            network='none', cap_drop=['ALL'], security_opt=['no-new-privileges:true'],
        )
        ...
        with tarfile.open(fileobj=archive, mode='w') as bundle:
            ...
            bundle.add(repo_root / 'tests/agent/fixtures/chat-provider.ts', arcname='pi-config/extensions/provider.ts')
