...
for (const width of [1440, 320]) {
  test(`expanded timing is concise, persisted and not visible when collapsed at ${width}px`, async ({ page, context }) => {
    ...
    await expect(page.getByText('Now checking a file.')).toBeVisible()
    ...
    await expect(group.locator('.tool__duration')).toHaveText('12.3s')
    await expect(nextGroup.getByRole('group', { name: 'Response timing' })).toHaveCount(0)
    await nextGroup.locator('summary').click()
    await expect(nextGroup.getByRole('group', { name: 'Response timing' })).toHaveText('Tools 12.5sThinking 3.2sReply 800msOther 250ms')
    await expect(nextGroup.locator('.tool__duration')).toHaveText('170ms')
    await expect(groups.locator('.tool__pulse')).toHaveCount(0)
    await expect(groups.locator('.tool-group__indicator-arc')).toHaveCount(0)
    await expect(group.locator('.tool-group__failed')).toHaveCount(0)
    await expect(nextGroup.locator('.tool-group__failed')).toHaveText('1 failed')
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
    await page.screenshot({ path: `test-results/tool-timing-${width}.png` })
    await group.locator('summary').click()
    await expect(breakdown).toHaveCount(0)
    await expect(nextGroup.getByRole('group', { name: 'Response timing' })).toBeVisible()
    await page.reload()
    await group.locator('summary').click()
    await expect(breakdown).toHaveText('Tools 12.5sThinking 3.2sReply 800msOther 250ms')
  })
