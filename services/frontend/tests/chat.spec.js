...
for (const width of [1440, 320]) {
  test(`project changes expand independently with relative paths and live counts at ${width}px`, async ({ page, context }) => {
    ...
    const projects = bar.locator('.project-changes__project')
    const summary = projects.first().locator(':scope > summary')
    const tree = projects.first().getByRole('list', { name: 'Changed files in StromFLIX/nautionette', exact: true })
    const secondTree = projects.nth(1).getByRole('list', { name: 'Changed files in StromFLIX/website', exact: true })
    await expect(projects).toHaveCount(2)
    await expect(summary).toContainText('+182')
    ...
    const frontendFile = tree.locator('[title="services/frontend/src/components/ProjectChanges.vue"]')
    await expect(frontendFile).toHaveText('ProjectChanges.vue')
    await expect(frontendFile).toBeVisible()
    await expect(projects.first()).toContainText('Binary')
    await expect(secondTree).not.toBeVisible()
    await projects.nth(1).locator(':scope > summary').click()
    await expect(secondTree).toBeVisible()
    const frontendFolder = tree.locator('summary[aria-label="Folder services/frontend"]')
    await frontendFolder.focus()
    await page.keyboard.press('Enter')
    await expect(frontendFile).not.toBeVisible()
    await expect(tree.locator('[title="services/backend/nautionette_backend/routers/chats.py"]')).toBeVisible()
    state.changes.alpha[0].additions = 200
    state.changes.alpha[0].files[0].additions = 198
    await expect(summary).toContainText('+200', { timeout: 10000 })
    await expect(tree).toBeVisible()
    await expect(frontendFile).not.toBeVisible() // polling preserves collapsed directories
    await frontendFolder.focus()
    await page.keyboard.press('Space')
    await expect(frontendFile).toBeVisible()
    await expect(tree).toContainText('+198')
...
test('project changes clear after push and refresh when an idle window regains focus', async ({ page, context }) => {
  const state = changeFixture()
  ...
  state.changes.alpha = state.changes.alpha.map((project) => ({ ...project, files: [], file_count: 0, additions: 0, deletions: 0 }))
  await page.evaluate(() => window.dispatchEvent(new Event('focus')))
  await expect(bar).not.toBeVisible()
  state.changes.alpha[0] = changeFixture().changes.alpha[0]
  await page.evaluate(() => window.dispatchEvent(new Event('focus')))
  await expect(bar.locator('.project-changes__project')).toHaveCount(1)
})
...
test('project changes retain stale counts on failure, recover, and clear on navigation', async ({ page, context }) => {
  ...
  await page.goto('/chats/alpha')
  const bar = page.locator('.project-changes')
  await expect(bar.locator('.project-changes__project')).toHaveCount(2)
...
for (const width of [1440, 320]) {
  test(`chat list distinguishes unread replies, progress and internet approval at ${width}px`, async ({ page, context }) => {
    ...
    const spinner = alpha.locator('.avatar__spinner')
    const arc = spinner.locator('.avatar__spinner-arc')
    await expect(spinner).toHaveCSS('animation-name', 'chat-activity-orbit')
    await expect(arc).toHaveCSS('animation-name', 'chat-activity-sweep')
    const dashArray = await arc.evaluate((el) => getComputedStyle(el).strokeDasharray)
    await expect.poll(() => arc.evaluate((el) => getComputedStyle(el).strokeDasharray)).not.toBe(dashArray)
    await expect(beta.locator('.avatar__spinner')).toHaveCount(0)
  ...
  test(`composer light follows progress and respects reduced motion at ${width}px`, async ({ page, context }) => {
    ...
    const spinner = page.locator('a[href="/chats/alpha"] .avatar__spinner')
    await expect(spinner).toHaveCSS('animation-name', 'none')
    await expect(spinner.locator('.avatar__spinner-arc')).toHaveCSS('animation-name', 'none')
  ...
  test(`running chats share one button for stop and queue at ${width}px`, async ({ page, context }) => {
    const state = initial()
    ...
    await page.goto('/chats/alpha')
    const action = page.locator('.composer__send, .composer__stop')
    await expect(action).toHaveCount(1)
    await expect(action).toHaveAccessibleName('Stop response')
    await expect(action).toBeEnabled()
    const stop = await action.boundingBox()
    await page.locator('textarea').fill('x')
    await expect(action).toHaveAccessibleName('Queue message')
    await expect(action).toBeEnabled()
    await expect(page.getByRole('button', { name: 'Stop response' })).toHaveCount(0)
    const queue = await action.boundingBox()
    expect(queue.x).toBeCloseTo(stop.x)
    expect(queue.width).toBe(stop.width)
    await page.locator('textarea').fill('   ')
    await expect(action).toHaveAccessibleName('Stop response')
    await page.locator('textarea').press('Enter')
    expect(state.stops).toEqual([]) // The send shortcut must never stop a response.
    ...
    await expect(action).toHaveAccessibleName('Stop response')
    expect(state.payloads[0].queue).toBe(true)
...
test('image-only drafts switch the running action between stop and queue', async ({ page, context }) => {
  const state = initial()
  state.models = [{ id: 'test/model', supports_images: true }]
