...
for (const width of [1440, 320]) {
  test(`system health exposes nested diagnostics at ${width}px`, async ({ page, context }) => {
    ...
    const workflowMcp = page.getByRole('article', { name: 'Workflow MCP' })
    await expect(workflowMcp.getByText('/data/drafts', { exact: true })).toBeHidden()
    await workflowMcp.getByText('Diagnostics', { exact: true }).click()
    await expect(workflowMcp.getByText('/data/drafts', { exact: true })).toBeVisible()
