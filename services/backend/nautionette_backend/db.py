...
class Database:
    ...
    def accept_chat_message(
        ...
        *, queue: bool = False,
        ...
        with self._lock, self._conn:
            ...
            waiting = queue and self._conn.execute(
    ...
    def next_chat_turn(self, chat_id: str) -> dict[str, Any] | None:
        with self._lock, self._conn:
            ...
            self._conn.execute(
                ...
                "rowid = (SELECT COALESCE(MAX(rowid), 0) + 1 FROM messages) WHERE id = ?", (turn["id"],),
            )
            return dict(turn)
    ...
    def consume_chat_input(self, turn_id: str, message_id: str) -> bool:
        with self._lock, self._conn:
            ...
            if changed:
                self._conn.execute(
                    "UPDATE messages SET meta = json_remove(meta, '$.queued') WHERE id = ?", (message_id,),
                )
